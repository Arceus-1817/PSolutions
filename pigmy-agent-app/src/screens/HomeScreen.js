...
export default function HomeScreen({ navigation }) {
    ...
    try {
      ...
      if (pendingTxns.length > 0) {
        for (const tx of pendingTxns) {
          if (tx.status === 'SKIPPED_CLOSED') {
            markTransactionSynced(tx.id);
            continue;
          }

          const endpoint = tx.type === 'EMI' ? '/api/transactions/loan-emi' : '/api/transactions/deposit';
          
          // 🚨 THE FIX: Perfectly formatted payload exactly like your web dashboard
          const payload = {
            customerId: parseInt(tx.customerId),
            agentId: agentId, // Spring Boot likely requires this!
  ...
  const handleLogout = async () => {
    Alert.alert(
      "Logout",
      "Do you want to clear all local data? (This will delete pending syncs!)",
      [
        ...
        { 
          ...
          style: "destructive", 
          onPress: async () => {
            clearLocalData(); // This now clears the pending_sync table too
